    ...
    try
     ...
     begin
       try
        ...
        Application.MessageBox('HATA OLUÞTU','UYARI',MB_OK+MB_ICONEXCLAMATION+MB_SYSTEMMODAL);
...
inherited;
  ...
  if SecimIDstr='' then
  begin
    ShowMessage('Sipariþ Seçmelisiniz.');
    exit;
  end;
...
inherited;
  ...
  if SecimIDstr='' then
  begin
    ShowMessage('Geri Alýnacak Sipariþi Seçmelisiniz.');
    exit;
  end;
   TRY
    with xAdoQuery Do
